...
// Defines
//-----------------------------------------------------------------------------
#define MAX_LONGFILENAME_ENTRIES	5
...
// Little Endian
	...
	#define GET_32BIT_WORD(buffer, location)        (*((unsigned long*) &buffer[location]))
	#define GET_16BIT_WORD(buffer, location)	(*((unsigned short*)&buffer[location]))

	#define SET_32BIT_WORD(buffer, location, value)	*((unsigned long*) &buffer[location])=value;
	#define SET_16BIT_WORD(buffer, location, value)	*((unsigned short*)&buffer[location])=value;
...
struct lfn_cache
{
	...
	unsigned char String[MAX_LONGFILENAME_ENTRIES+1][MAX_LFN_ENTRY_LENGTH];
	unsigned char Null;
#endif
